...
// burn'ün async, kibar nonos versiyonu:
async fn burn_polite(secs: u64) -> u64 {
    ...
    while start.elapsed() < Duration::from_secs(secs) {
        ...
        tokio::task::yield_now().await;   // ← her batch sonunda: "worker'ı teslim ediyorum,
    }                                     //    sırada bekleyen varsa koşsun, sonra bana dön"
    count
}
...
async fn main() {
    ...
    match mode.as_str() {
        "spawn"    => { for _ in 0..2 { tokio::spawn(async { burn_polite(5).await }); } }
        _          => {} // kontrol: burn yok
    }
