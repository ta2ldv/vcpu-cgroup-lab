// ═══════════════════════════════════════════════════════════════════════
// RespPress — step06: CLI flag'leri + sabit rate + summary + dinamik tablolar
// (v1'in FİNAL hali — Single-Thread PoC burada mühürlendi)
// ═══════════════════════════════════════════════════════════════════════
//
// BÜYÜK RESİM — bu program ne yapar:
//   Redis'e (ya da RESP konuşan herhangi bir server'a) TEK bağlantıdan
//   yük basar ve ölçer: SET/GET karışımını batch'ler halinde gönderir,
//   cevapları RESP dilbilgisiyle tek tek ayıklar, her isteğin latency'sini
//   kaydeder ve sonunda üç tablo basar: summary + latency + histogram.
//
// KOMUT SATIRI (hepsi opsiyonel, sırasız):
//   -n, --number      toplam istek         (default 10000)
//   -b, --batch-size  paket boyu           (default 1 = ping-pong)
//   -r, --rate        hedef req/s          (default 0 = unlimited/yardır)
//   -t, --target      host[:port]          (default 127.0.0.1:6379;
//                                           port yoksa :6379 eklenir)
//   Kurallar: aynı flag iki kez → ERROR (duplicate guard; kısa/uzun karışık
//   olsa bile). Bilinmeyen flag / bozuk değer / değersiz flag → ERROR+usage.
//   Default kalan her düğme summary'de "(default)" işareti taşır — rapor,
//   neyin BİLİNÇLİ seçildiğini de belgeler.
//
// AKIŞ, adım adım (aptala anlatır gibi):
//   1. parse_args(): argümanları oku, default'ları doldur, host/port ayır.
//   2. PID + banner bas (koşu sırasında dışarıdan ps -T ile izlenebilsin).
//   3. TCP bağlantısı aç (tokio, async — bağlanana kadar worker serbest).
//   4. batch kur: SET,GET,SET,GET... frame'leri TEK buffer'a yapıştırılır.
//      (pipelining'in tüm sırrı bu yapıştırma: N istek aynı yolculuğa biner,
//       RTT maliyeti N'e bölünür. 5.2'de printf|nc ile elle yapılanın aynısı.)
//   5. Her turda:
//      a. rate>0 ise TAKVİME uy: bu batch'in planlanan gönderim anı
//         start + tur × aralık. Erken geldiysen uyu; GEÇ KALDIYSAN UYUMA
//         ama saati yine plandan başlat → COORDINATED OMISSION ilkesi:
//         server seni geriletirse o gecikme latency'ye YAZILIR, gizlenmez.
//      b. batch'i tek write'ta gönder.
//      c. batch_size kadar CEVAP toplanana dek oku: read'ler acc buffer'ına
//         eklenir, parse_one() oradan tam cevapları ayıklar (kısmi frame →
//         None → biraz daha oku). Her tamamlanan cevap: latency kaydı
//         (plan/gönderim anından şimdiye) + ok/error sayacı.
//      d. Guard'lar: read()==0 → server kapattı → ERROR+çık;
//         5 sn sessizlik → timeout → ERROR+çık; -ERR cevabı → ÇÖKME,
//         errors++ ve devam (say-ve-devam-et; ilk hata örneği saklanır).
//   6. Rapor: summary tablosu (config echo + duration/achieved/replies),
//      latency tablosu (min/p50/p90/p99/p99.9/max/avg — kolonlar İÇERİĞE
//      göre genişler, 6 haneli µs değerleri çerçeveyi asla kırmaz),
//      histogram (█ + 1/8'lik uç blokları ▏▎▍▌▋▊▉ → %0.25 hassasiyet).
//
// FİNAL DOĞRULAMA ÖLÇÜMLERİ (t3.large, localhost, 2026-08-22):
//   -n 1000000 -b 100 -r 2000000 (hedef>kapasite, CO gösterisi):
//       achieved 881k < target 2M; p50=317950µs(!), max=635006µs,
//       histogram %98.5 >10ms — "hedef kapasiteyi aşınca kuyruk birikir ve
//       CO muhasebesi bunu DÜRÜSTÇE yazar" dersinin kanıtı. Tablo 6 haneli
//       değerlerle düzgün açıldı (dinamik genişlik testi).
//   -n 1000 -b 10 (küçük): p50=60µs, tablo kompakt kaldı.
//   -n 200000 -b 100 -r 20000: duration 10.00s, achieved 20007 —
//       takvim hükmü kanıtı (200k istek ÷ 20k/s = tam 10 saniye).
//
// TEST GEÇMİŞİ (T1-T11, hepsi geçti): help · default'lar · yardır · rate ·
//   CO · target/port ayrımı · kısa/uzun flag kombinleri · sıra bağımsızlığı ·
//   kısmi flag'ler · 4 negative test (unknown/--n/bozuk değer/değersiz) ·
//   2 duplicate guard testi. Şahitlik: RESETSTAT → koşu → INFO stats farkı
//   = n+1 (kuruş kuruş, defalarca).
//
// V1'İN BİLİNÇLİ SINIRLARI (v2'nin işleri):
//   - TEK bağlantı (concurrency yok) — v2'nin tek farkı N bağlantı olacak,
//     böylece v1↔v2 kıyası SAF concurrency etkisini ölçer.
//   - workload sabit (SET/GET 50/50, tek key 'foo' → DBSIZE hep 1;
//     sözlük/memory maliyeti hiç ölçülmez) — key çeşitliliği + komut
//     karışımı düğmeleri ileride.
//   - GET içerik doğrulaması yok (parser geçerliliğe bakar, değere değil).
//
// Koşu örnekleri:
//   cargo run --release -- --help
//   cargo run --release -- -n 200000 -b 100 -r 20000
//   cargo run --release -- --number 50000 --batch-size 10 -t 10.0.0.42:6379
// ═══════════════════════════════════════════════════════════════════════
